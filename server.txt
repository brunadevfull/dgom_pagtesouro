...
var rootCas = require('ssl-root-cas').create(); // ALT CB GERRY
const fs = require('fs');// ALT CB GERRY
...
const app = express();
const port = 3000;
https.globalAgent.options.ca = rootCas; // ALT CB GERRY
...
// SEGURANÇA => REDIRECIONAR REQUISIÇÃO NA PORTA 3000 PARA O INDEX
/*
app.use((req, res, next) => {
  if (req.url === '/') {
    return res.redirect('https://pagtesouro.dgom.mb/pagtesouro/');
  }
  next();
});
*/
...
res.setHeader('Access-Control-Allow-Origin', '127.0.0.1');
	...
	// Permitir credenciais (cookies, cabeçalhos de autorização, etc.) se necessário
	res.setHeader('Access-Control-Allow-Credentials', 'true');
 	// Continue com a próxima camada do middleware
	next();
});
...
var corsOptions = { // ALT CB GERRY
  origin: false // ALT CB GERRY
} // ALT CB GERRY
...
// router.post('/handle',(request,response) => { ANTERIOR AO SSL
app.post('/handle', cors(corsOptions), (request,response) => { // ALT
...
//value.urlRetorno =  "https://valpagtesouro.tesouro.gov.br/simulador";
//value.urlNotificacao = "https://valpagtesouro.tesouro.gov.br/api/simulador/ug/notificacao";  
		...
		function montaref(result) {
			...
			function enviapost(){
					...
					const options = {
						...
						host: 'proxy-1dn.mb',
						port: 6060
						//,auth: {username: 'xxxxxxxxxxxx', password: 'xxxxxxxxxx'}
					 }
					};
					...
					try {
						...
						//var encryptionKey = Buffer.from('xxxx', 'hex');
						...
						//var buf2 = cipher.final('base64');
						//var nome_encrypted = Buffer.concat([buf1, buf2])
						cipher  = crypto.createCipheriv( 'aes-128-cbc', key, iv );
						var cnpjCpf_encrypted = cipher.update(value.cnpjCpf, 'utf8','base64');
						cnpjCpf_encrypted += cipher.final('base64');
						//buf1 = cipher.update( Buffer.from(value.cnpjCpf) );
						//buf2 = cipher.final();
						//var cnpjCpf_encrypted = Buffer.concat([buf1, buf2])
...
//router.post('/update',cors(corsOptions),(request,response) => { // ALT CB GERRY
app.post('/update', cors(corsOptions),(request,response) => {
	...
	const pool = new Pool({
		user: 'xxx',
		host: 'xx', // NOVO BANCO POSTGRESQL v12
		database: 'xxx',
		schema: 'xxx',
		password: 'xxxx',
		port: 5432
	...
	try {
		...
		if (request.body.cat_servico == "CCIM") {
										...
										var options = {
										  ...
										  cert: fs.readFileSync('/var/www/html/pagtesouro/recim-chain.pem') // ALT CB GERRY
										
										};
										
										https.globalAgent.options.ca = rootCas + fs.readFileSync('/var/www/html/pagtesouro/recim-chain.pem') ;
										  ...
										  res.on('data', (d) => {
											...
											if ( (e["sucesso"] == true) || (e["sucesso"] == false && e["erro_Msg"] == 'Pagamento registrado anteriormente')) {
												...
												geralog("Enviada resposta ao usuário.");  
													...
													//connection.connect();
													/* var query = connection.query('UPDATE pagtesouro.TB_PGTO  set 	SINGRA_OK = 1 where ID_PGTO = ?',[request.body.id_pgto],function (error, results, fields) {
													geralog(query.sql);
													if (error) geralog("Erro no registro! " + error);
													else geralog("Atualização no Banco de Dados efetuado com sucesso! (MYSQL)");	
													}); */
							...
							/* 	
								//mysql
								var query = connection.query('UPDATE pagtesouro.TB_PGTO  set 	DS_TP_PGTO = ?,	VR_PAGO = ?, DS_NOMEPSP = ?,  CD_TRANSACPSP = ?,  DS_SITUACAO = ?,   DT_SITUACAO = STR_TO_DATE(?,"%Y-%m-%dT%H:%i:%sZ")  where ID_PGTO = ?',[resposta["tipoPagamentoEscolhido"],resposta["valor"],resposta["nomePSP"],resposta["transacaoPSP"],resposta["situacao"]["codigo"],resposta["situacao"]["data"],request.body.id_pgto],function (error, results, fields) {									
									geralog(query.sql);
									if (error) throw (new Date().toLocaleString()+" Erro no registro!");
									geralog("Atualização no Banco de Dados efetuado com sucesso!");		
								}); */
								...
								/* var query = connection.query('UPDATE pagtesouro.TB_PGTO  set 	DS_TP_PGTO = ?,	VR_PAGO = ?,  DS_SITUACAO = ?,   DT_SITUACAO = STR_TO_DATE(?,"%Y-%m-%dT%H:%i:%sZ")  where ID_PGTO = ?',[resposta["tipoPagamentoEscolhido"],resposta["valor"],resposta["situacao"]["codigo"],resposta["situacao"]["data"],request.body.id_pgto], function (error, results, fields) {
									geralog(query.sql);
									if (error) throw (new Date().toLocaleString()+" Erro no registro!");
									geralog("Atualização no Banco de Dados efetuado com sucesso!");									
